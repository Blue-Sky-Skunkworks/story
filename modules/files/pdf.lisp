...
(defun extract-pdf-page (file page &key ocr (destination (story-file "modules/files/tmp-pdf-pages/")))
  (ensure-directories-exist destination)
  (let ((name (f "~A~A-p~A" destination (pathname-name file) page)))
    (run `(pdfimages "-png" "-f" ,page "-l" ,page ,file ,name))
    (let ((img-name (f "~A-000.png" name)))
      (when ocr (run `(tesseract ,img-name ,(f "~A-ocr.txt" img-name))))
      img-name)))
